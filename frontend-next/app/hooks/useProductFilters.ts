/**
 * Хук для фильтрации и сортировки продуктов
 * Client Component hook - использует useState, useCallback, useMemo
 */
'use client';
...
export const useProductFilters = ({ products }: UseProductFiltersProps) => {
  ...
  // Мемоизированные сеттеры с debounce для поиска
  const setSearchQuery = useCallback((query: string) => {
    // Обновляем сразу для UI отклика
    ...
    // Debounce для тяжелых вычислений фильтрации
    // Но так как фильтрация уже мемоизирована, это не критично
    // Оставляем для будущих оптимизаций
  }, []);
  ...
  // Мемоизируем строковые представления для стабильности зависимостей
