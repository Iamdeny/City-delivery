...
import RecommendationProduct from '@/app/components/cart/RecommendationProduct';
...
const CartModal: React.FC<CartModalProps> = ({
  ...
  const [deliveryStatus, setDeliveryStatus] = useState<CartDeliveryStatus>({
    ok: null,
    message: null,
  });
  ...
  const trackEvent = useCallback(
    async (eventType: 'impression' | 'click', productId: number) => {
      try {
        await fetch('/api/analytics/recommendation', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            eventType,
            productId,
            userId: null, // замените на реальный userId, если есть
  ...
  const handleCheckoutClick = useCallback(() => {
    ...
    if (deliveryStatus.ok === false) {
      ...
      onShowNotification(
        String(deliveryStatus.message || 'Доставка недоступна'),
        'error'
      );
      return;
    }
    ...
    try {
      ...
      if (!hasStore || !hasCoords) {
        setDeliveryPickerOpen(true);
        onShowNotification(
          'Перед оформлением укажите адрес и выберите склад.',
          'info'
        );
        return;
      }
            ...
            <div className='px-5 pt-[calc(var(--safe-top)+16px)] pb-4 border-b border-gray-100 bg-white sticky top-0 z-10'>
              ...
              <div className='mt-3 flex items-center gap-3 rounded-2xl bg-[#f0f2f5] px-4 py-3'>
                ...
                <div className='flex-1 min-w-0'>
                  <span className='text-[15px] font-semibold text-[#1a1a1a] block'>
                    Самокат · {deliveryTime}
                  </span>
                  <span className='text-[13px] text-[#5a5a5a] block truncate'>
                    {deliveryAddress}
                  </span>
                </div>
              </div>
                    ...
                    {loadingRecommendations ? (
                      <div className='flex gap-3 overflow-x-auto pb-1'>
                        {[1, 2, 3].map((i) => (
                          <div
                            key={i}
                            className='flex-shrink-0 w-[120px] rounded-2xl overflow-hidden bg-[#f0f2f5] animate-pulse'
                          >
                  ...
                  <div className='px-5 py-4 space-y-3 border-t border-gray-100'>
                    ...
                    >
                      <div className='flex-1 min-w-0'>
                        ...
                        <div className='mt-0.5 text-[13px] font-normal text-gray-500'>
                          Нужно будет войти в профиль
                        </div>
                      </div>
                      ...
                      <div className='flex items-start gap-3 flex-1 min-w-0'>
                        ...
                        <div className='flex-1 min-w-0'>
                          ...
                          <div className='mt-0.5 text-[13px] font-normal text-green-800'>
                            Войдите по Сбер ID, чтобы получать и списывать
                            бонусы
                          </div>
                        </div>
                      </div>
