...
Чтобы открыть приложение на телефоне в той же Wi‑Fi сети:
...
2. **Узнайте IP компьютера в локальной сети:**
   - Windows: в терминале `ipconfig`, смотрите «IPv4-адрес» у активного адаптера (обычно Wi‑Fi).
   - Пример: `192.168.1.105`.

3. **На телефоне** в браузере откройте: `http://ВАШ_IP:3000`  
   Например: `http://192.168.1.105:3000`.

4. **Если нужен backend** (заказы, корзина, авторизация): запустите backend так, чтобы он слушал все интерфейсы (например `0.0.0.0`), и в корне `frontend-next` создайте `.env.local`:
   ```
   NEXT_PUBLIC_API_URL=http://ВАШ_IP:5000
   ```
   Подставьте тот же IP, что и в п. 2. После изменения переменных перезапустите `npm run dev:lan`.
